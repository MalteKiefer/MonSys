  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-start gap-3 p-4">
          ...
          {(h.groups ?? []).length > 0 && (
            <div className="flex flex-wrap items-center gap-1">
              <span className="text-[11px] font-medium uppercase tracking-wider text-fg-subtle">{t("hostDetail:overview.groupsLabel")}</span>
              {(h.groups ?? []).map((g) => (
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard label={t("hostDetail:overview.statCpuCores")} value={h.cpu_cores} />
        <StatCard label={t("hostDetail:overview.statRam")} value={formatBytes(h.ram_total_bytes)} />
        <StatCard label={t("hostDetail:overview.statFirstSeen")} value={new Date(h.first_seen_at).toLocaleDateString()} />
        <StatCard label={t("hostDetail:overview.statStatusSince")} value={h.status_since ? relativeTime(h.status_since, t) : "—"} />
      </div>

      <div className="flex items-center justify-between">
        <h2 className="text-[11px] font-semibold uppercase tracking-wider text-fg-subtle">{t("hostDetail:overview.liveSystem")}</h2>
  ...
  return (
    <div className="flex flex-wrap items-center gap-1">
      <span className="text-[11px] font-medium uppercase tracking-wider text-fg-subtle">
        <Tag className="mr-1 inline h-3 w-3" /> {t("hostDetail:overview.tagsLabel")}
      </span>
      {tags.map((tag) => (
      ...
      {save.isError && <span className="text-[10px] text-fail">{t("hostDetail:overview.saveFailed")}</span>}
    </div>
  );
}
...
function SystemPanel({ samples, ramTotal, loading }: { samples: SystemSample[]; ramTotal: number; loading: boolean }) {
  ...
  const matrix = useMemo(() => {
    const times = samples.map((s) => Math.floor(new Date(s.time).getTime() / 1000));
    const cpu = samples.map((s) => s.cpu_usage_pct);
    const ram = samples.map((s) => (ramTotal > 0 ? (s.ram_used_bytes / ramTotal) * 100 : 0));
    return [times, cpu, ram];
  }, [samples, ramTotal]);

  const series: ChartSeries[] = [
    { label: t("hostDetail:overview.statCpu"), color: colorFor(0), fill: "rgba(16,185,129,0.10)" },
    { label: t("hostDetail:overview.statRam"), color: colorFor(1), fill: "rgba(96,165,250,0.10)" },
  ];
    ...
    <Panel>
      <PanelHeader>
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">{t("hostDetail:overview.systemTitle")}</h3>
        </div>
      </PanelHeader>
      <PanelBody>
        {loading && samples.length === 0 ? (
          ...
          <Empty>{t("hostDetail:overview.noSamples")}</Empty>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
              <StatCard label={t("hostDetail:overview.statCpu")} value={`${latest.cpu_usage_pct.toFixed(1)}%`} hint={relativeTime(latest.time, t)} />
              <StatCard label={t("hostDetail:overview.statRam")} value={`${ramPct.toFixed(0)}%`} hint={formatBytes(latest.ram_used_bytes)} />
              <StatCard label={t("hostDetail:overview.statLoad")} value={`${latest.load_1.toFixed(2)} / ${latest.load_5.toFixed(2)} / ${latest.load_15.toFixed(2)}`} />
              <StatCard label={t("hostDetail:overview.statSwap")} value={formatBytes(latest.swap_used_bytes)} />
              <StatCard label={t("hostDetail:overview.statUptime")} value={formatUptime(latest.uptime_sec)} />
            </div>
            <div className="mt-4">
